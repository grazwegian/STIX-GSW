...
function stx_fsw_sd_spectrogram2ospex, spectrogram, specpar = specpar, time_shift = time_shift, ph_energy_edges = ph_edges, fits = fits, plotman_obj = pobj, specfilename = specfilename, srmfilename  = srmfilename,$
  flare_location = flare_location,  gtrans32 = gtrans32, livetime_fraction = livetime_fraction, xspec = xspec, _extra = _extra
  ...
  if keyword_set(fits) then begin
    ...
    stx_write_ospex_fits, spectrum = spectrum_in, srmdata = srm,  specpar = specpar, time_shift= time_shift, srm_atten =srm_atten, specfilename = specfilename, $
       srmfilename = srmfilename, ph_edges = ph_edges, xspec = xspec, sys_err = sys_err
