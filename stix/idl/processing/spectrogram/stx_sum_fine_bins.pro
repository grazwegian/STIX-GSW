...
;
;    livetime : in, required, type="float array"
...
function stx_sum_fine_bins, spectrogram, livetime
  ...
  bin_file = loc_file( 'summed_bins_20240319.txt', path = getenv('STX_DET') )

  fine_e = stx_read_summed_bins(bin_file)
  ...
  n_times = n_elements(spectrogram.time_axis.duration)

  e_axis_summed = stx_construct_energy_axis(energy_edges = fine_e, select = indgen(n_elements(index[0,*])+1))
  energy_edges_used = where_arr(fix(10*spectrogram.energy_axis.edges_1), fix(10*e_axis_summed.edges_1))
